...
**Official recommended (Native):**
```bash
curl -fsSL https://claude.ai/install.sh | bash
```
macOS/Linux/WSL. Installs to a system-managed location, auto-updates in background.

**Alternative: npm (works in restricted networks):**
```bash
...
If behind a proxy:
```bash
npm install -g @anthropic-ai/claude-code --proxy http://127.0.0.1:7891 --https-proxy http://127.0.0.1:7891
```

### Network Notes (China / Restricted Environments)

- `downloads.claude.ai` (native install CDN) is often blocked by GFW — SSL errors or timeouts are common.
- `claude.ai` redirects to "App unavailable in region" without a working proxy.
- The npm fallback (`@anthropic-ai/claude-code`) usually works through a proxy since it downloads from npm registry (which is accessible via proxy or CDN mirror).
- After npm install, verify with `claude --version` and `which claude`.

### Auth Options

- **API key (recommended for CLAUDE.md/headless):** `claude auth login --console` — prompts for an Anthropic Console API key. Also works with `ANTHROPIC_API_KEY` env var.
- **OAuth (interactive):** `claude auth login` — opens browser for Pro/Max/Team/Enterprise login. Needs direct access to claude.ai.
